...
category_schema = Schema(doc=ID(stored=True), name=TEXT, description=TEXT)
product_schema = Schema(doc=ID(stored=True), name=TEXT, description=TEXT,
                        reference=KEYWORD, keywords=KEYWORD)
...
for c in Category.objects:
    for lg in langs:
        category_writer.add_document(
                            doc=unicode(c.id),
                            name=c.name.get(lg, u''),
                            description=c.description.get(lg, u'')
                            )
